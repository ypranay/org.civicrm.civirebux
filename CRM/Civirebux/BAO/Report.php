...
class CRM_Civirebux_BAO_Report{
	...
	*/
	public static function save(){
		...
		$ret = array();
		if($oldid == 0){
			$sql = "INSERT INTO civicrm_civirebux_configuration (`id`,`name`,`renderer`,`aggregator`,`vals`,`rows`,`cols`,`time`,`desc`)
			VALUES (NULL,'".$name."','".$renderer."','".$aggregator."','".$vals."','".$rows."','".$cols."','".$time."','".$desc."')";		
	...
	public static function getDataForSavedReports(){
		$sql = "SELECT `id`, `name`, `desc`, `time` FROM civicrm_civirebux_configuration";
        	$dao = CRM_Core_DAO::executeQuery($sql);
        	$array_configs = array();
        	while($dao->fetch()){
                	$config = array();
